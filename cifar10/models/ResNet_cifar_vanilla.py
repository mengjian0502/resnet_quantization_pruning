...
class filterpruneConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, padding=0, dilation=1, groups=1, bias=True):
        super(filterpruneConv2d, self).__init__(in_channels, out_channels, kernel_size,
    ...
    def forward(self, input):
        ...
        # weight = self.weight[self.w_idx.tolist(), :, :, :]
        weight = self.weight.clone()
        weight[self.w_zero.tolist(), :, :, :] = 0.0
        # directly prune
    ...
    def get_weight_idx(self):
        ...
        num_keep = int(16 * 0.25) * int(out_channels/16)
...
class pLinear(nn.Linear):
    def __init__(self, input_features, output_features, bias=True):
        super(pLinear, self).__init__(input_features, output_features)
    ...
    def forward(self, input):
        ...
        weight[:, self.w_zero.tolist()] = 0.0
        #weight = self.weight[:, self.w_idx.tolist()]
        # input_keep = input[ :, self.w_idx.tolist()]
    ...
    def get_weight_idx(self):
        ...
        num_keep = int(16 * 0.25) * int(in_channels/16)
        #num_keep = int(out_channels * 0.9)
        # print(num_keep)
        idx = arg_max[:num_keep]
        self.w_idx, _ = idx.sort()
