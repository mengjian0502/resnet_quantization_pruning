...
def glasso_reshape(w_l, group_ch):
    kw = w_l.size(2)
        ...
        if args.swp:
            ...
            for m in model.modules():
                if isinstance(m, nn.Conv2d):
                    if not count in [0]:
                        w_l = m.weight
                        num_channel = w_l.size(1)
                        sub_grp_size = num_channel // 2
                        
                        w_l_g1 = w_l[:, :sub_grp_size, :, :]
                        w_l_g2 = w_l[:, -sub_grp_size:, :, :]

                        w_l_g1 = glasso_reshape(w_l_g1, sub_grp_size)
                        w_l_g2 = glasso_reshape(w_l_g2, sub_grp_size)

                        reg_g1 += glasso_thre(w_l_g1, 1)
                        reg_g2 += glasso_thre(w_l_g2, 1)
